...
# Codex surfaces exit-code-2 blocking reasons from stderr.
exec 1>&2

# Codex passes tool input as JSON on stdin.
INPUT=$(cat)
...
if echo "$COMMAND" | grep -qE '(terraform|terragrunt)\s+destroy'; then
  echo "BLOCKED: 'terraform destroy' is not allowed via an AI coding agent."
  echo ""
  ...
  echo "  3. Run 'terraform destroy' directly in your terminal (not via an agent)"
  echo ""
...
if echo "$COMMAND" | grep -qE 'kubectl\s+delete\s+(namespace|ns)\s+petclinic-prod'; then
  echo "BLOCKED: Deleting the production namespace is not allowed via an AI coding agent."
  echo ""
...
if echo "$COMMAND" | grep -qE 'kubectl\s+delete\s+(deployment|deploy|service|svc|ingress|ing|secret|configmap|cm|pvc|persistentvolumeclaim|daemonset|ds|statefulset|sts)\b' && \
  ...
  echo "BLOCKED: 'kubectl delete ${RESOURCE_TYPE}' in petclinic-prod is not allowed via an AI coding agent."
  echo ""
